...
class Airport {
    ...
    landPlane = (weather, plane) => {
        if (!this.airportPlanes.includes(plane) && this.airportPlanes.length < this.capacity && weather < 0.85 && plane instanceof Airplane) {
            console.log("Plane landed!");
            this.airportPlanes.push(plane);
            plane.currentAirport = this;
        }
        else {
            console.log("A plane with that ID has just left the airport or the weather is stormy or you are not flying a plane.")
        }
    }
